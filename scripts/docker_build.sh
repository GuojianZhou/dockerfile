...
case ${DOCKER_IMAGE} in
	...
	"CUDA-9.0-bazel-TF")
		...
		;;
	"CUDA-8.0")
		pushd ${TOP_DIR}/CUDA/CUDA-8.0.CUDNN-7.2
		cp ${TOP_DIR}/images/ubuntu-base-16.04.5-base-amd64.tar.gz .
		cp ${TOP_DIR}/images/bazel-0.5.4-installer-linux-x86_64.sh .
		cp -a ${TOP_DIR}/images/py .
		echo "The nv/cuda-8.0_cudnn-7.2_bazel_TF-1.4:V1.0 docker image will be created!"
		docker build -t nv/cuda-9.0_cudnn-7.2_bazel_TF-1.10:V1.0 .
		echo "The nv/cuda-8.0_cudnn-7.2_bazel_TF-1.4:V1.0 docker image has be created!"
		if [ -f ubuntu-base-16.04.5-base-amd64.tar.gz ]; then
			rm -f ubuntu-base-16.04.5-base-amd64.tar.gz
		fi
		if [ -f bazel-0.5.4-installer-linux-x86_64.sh ]; then
